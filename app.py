...
with tabs[-1]:
    ...
    # The direct URL to the dashboard.html file on GitHub Pages or raw
    # Since Streamlit doesn't serve static HTML files, we use a direct link
    
    # For the deployed version, you can use a direct link to a hosted version
    # But for now, we'll provide a download option
    
    st.markdown("""
    <div style="text-align: center; padding: 40px; background: #1e1e2a; border-radius: 10px;">
        <p style="margin-bottom: 20px; color: #ccc;">
            The 3D IFC Viewer requires a separate HTML file to function properly.<br>
            <strong>Click the button below to open the 3D viewer in a new tab.</strong>
        </p>
    """, unsafe_allow_html=True)
    
    # Create a direct link to the dashboard.html file
    # Since GitHub raw URLs don't work for CORS, we use Google Drive hosted version
    viewer_url = f"https://raw.githubusercontent.com/joelriley70-commits/ifc-dashboard/master/dashboard.html?building={project_folder}"
    ...
    with col2:
        ...
        <a href="{viewer_url}" target="_blank">
            <button style="
                ...
                font-weight: bold;
                width: 100%;">
                🔓 Open 3D Building Viewer in New Tab
            </button>
        </a>
        """, unsafe_allow_html=True)
    ...
    <p style="text-align: center; margin-top: 20px; color: #888; font-size: 12px;">
        💡 <strong>Instructions:</strong> Once opened, use the dropdown in the viewer sidebar to switch between buildings.<br>
    ...
    with st.expander("📥 Having trouble viewing? Download the HTML file to run locally", expanded=False):
        st.markdown("""
        1. Download the `dashboard.html` file below
        2. Place it in your `C:\\ifc_dashboard\\` folder
        3. Run `python -m http.server 8000` in that folder
        4. Open `http://localhost:8000/dashboard.html` in your browser
        
        This ensures the 3D viewer works correctly with your local files.
        """)
        
        # Read the dashboard.html file and offer for download
        try:
            ...
            st.warning("dashboard.html file not found locally")
# =====================================================
